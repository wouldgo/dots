...
function __load_python () {
  ...
  if [ "${PATH_TO_REQUIREMENTS_FILE}" != '/' ] && [ "${PATH_TO_PYRC_FILE}" != '/' ]; then
    ...
    if [ "$_PYTHON_VERSION" != "$_ACTUAL_PYTHON_VERSION" ]; then
      rtx use "node@${_PYTHON_VERSION}"
    fi

    PYTHON_EXEC=$(rtx which python)
    PIP_EXEC=$(rtx which pip)

    if [ ! -d "${PATH_TO_REQUIREMENTS_FILE}/${_DOT_VENV_DIRECTORY}" ];  then

      mkdir -p "${PATH_TO_REQUIREMENTS_FILE}/${_DOT_VENV_DIRECTORY}";
      "${PYTHON_EXEC}" -m venv "${PATH_TO_REQUIREMENTS_FILE}/${_DOT_VENV_DIRECTORY}";
    fi

    # shellcheck source=/dev/null
    source "${PATH_TO_REQUIREMENTS_FILE}/.venv/bin/activate";
